...
def get_list(link, title):
    ...
    root1.title(title)
    root1.resizable(0, 0)
    sf = ScrolledFrame(root1, width=520, height=650)
...
def get_top20():
    ...
    title = "Soju-Scraper V5 - Top K-Dramas"
    get_list(link, title)
...
def get_upcoming():
    ...
    title = "Soju-Scraper V5 - Upcoming K-Dramas"
    try:    
        get_list(link, title)
    except IndexError:
        pass
...
def get_newest():
    ...
    title = "Soju-Scraper V5 - Latest K-Dramas"
    get_list(link, title)
...
btn_search = Button(frm_search, text="Search", command=lambda:threading.Thread(target=get_info).start())
btn_search.grid(row=0, column=2, padx=10, pady=10)
root.bind('<Return>',lambda event:threading.Thread(target=get_info).start())
