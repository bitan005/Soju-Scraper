...
def get_list(link, title):
    ...
    top_20_window.title(title)
    top_20_window.resizable(0, 0)
    for i in range(10):
        Label(top_20_window, text=f"#{i+1} {top_20[i].get_text()}  {bit_of_info[i].get_text()}",
              justify=LEFT, width=35, borderwidth=1, relief="solid").grid(row=i, column=0, padx=5, pady=5)
        Label(top_20_window, text=f"#{i+11} {top_20[i+10].get_text()}  {bit_of_info[i+10].get_text()}", 
              justify=LEFT, width=35, borderwidth=1, relief="solid").grid(row=i, column=1, padx=5, pady=5)
              
    top_20_window.mainloop()

def get_top20():
    link = "https://mydramalist.com/shows/top_korean_dramas"
    title = "Soju-Scraper V3 - Top 20 K-Dramas"
    get_list(link, title)
